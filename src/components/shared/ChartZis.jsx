  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 font-['Manrope']">
      <div className="flex items-start justify-between mb-6 flex-wrap gap-4">
        <div>
          <h2 className="text-base font-bold text-gray-900">Tren Transaksi ZIS</h2>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="flex items-center gap-1 bg-gray-100 rounded-lg p-1">
            {["Zakat Maal", "Zakat Fitrah Uang", "Zakat Fitrah Beras", "Infaq", "Sedekah"].map((opt) => (
              <button
                key={opt}
                type="button"
                onClick={() => setKategori(opt)}
                className={`px-3 py-1.5 rounded-md text-xs font-semibold transition-all ${
                  kategori === opt ? "bg-[#10B981] text-white" : "text-gray-500 hover:text-gray-700"
                }`}
              >
                {opt}
              </button>
      ...
      <ResponsiveContainer width="100%" height={260}>
        <AreaChart data={chartData} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
          <defs>
            <linearGradient id="gradZIS" x1="0" y1="0" x2="0" y2="1">
          ...
          <YAxis hide />
          <Tooltip content={customTooltipFormatter(kategori)} />
          <Legend wrapperStyle={{ fontSize: 12, paddingTop: 16 }} />
          <Area type="monotone" dataKey="pemasukan" name="Pemasukan" stroke={themeColors.accent} strokeWidth={3} fill="url(#gradZIS)" dot={false} />
